...
class Character:
    def __init__(self):
        ...
        self.all_abilities = {'skills': self.__skills, 'perks': self.__perks, 'talents': self.__talents,
                              'complications': self.__complications, 'armor': self.__armors,
                              'cyberwear': self.__cyberwear, 'items': self.__items, 'weapons': self.__weapons}
    ...
    def InitNeeds(self):
        self.set_stat("Tiredness",
                      0)  # Increases every hour by 1 point, after 12 hours awake char will become affected by tiredness
        # And must throw int + roll against tiredness value to stay awake. sleeping will give 1/2 rec
        # back every two sleepinging hours
        self.set_stat("Hunger",
                      0)  # Will increase by 1/2 of body attribute per hour. eating will give 1/2 of rec + mods in food
    ...
    def giveRandomName(self, gender='undecided'):
        ...
        try:
            ...
            self.__attributes["Full name"] = str.capitalize(first_name) + " " + str.capitalize(
                second_name) + " " + first_nick + " " + last_nick + " " + str.capitalize(last_name)
    ...
    def save(self):
        ...
        full_name = self.get_attribute('First name') + ' ' + self.get_attribute(
            'Second name') + ' ' + self.get_attribute('Alias') + ' ' + self.get_attribute('Last name')
        if player != 'npc':
